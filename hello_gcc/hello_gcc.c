...
{
  for (uint32_t pos = 0; pos < 30*80; pos++) characters[pos] = 32;
  xpos = 0;
  ypos = 0;
  normal();
}
...
void addline(void)
{
  ...
  {
    uint32_t pos;
    for (pos=80;    pos < 30*80; pos++) characters[pos-80] = characters[pos];
    for (pos=29*80; pos < 30*80; pos++) characters[pos] = 32;
  }
...
{
  if (xpos > 79) { addline(); xpos = 0; }
  characters[xpos + ypos*80] = character | textmarker;
  xpos++;
}
